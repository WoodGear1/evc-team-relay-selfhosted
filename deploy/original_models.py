...
from sqlalchemy import Boolean, JSON, DateTime, ForeignKey, Index, String, Text, UniqueConstraint, func
...
class AuditAction(str, Enum):
    ...
    WEB_ACCESS_GRANTED = "web_access_granted"
...
class PublishedLink(Base, TimestampMixin):
    ...
    access_mode: Mapped[LinkAccessMode] = mapped_column(
        PgEnum(LinkAccessMode, name="linkaccessmode", create_constraint=False),
        nullable=False, server_default="public",
    )
    state: Mapped[LinkState] = mapped_column(
        PgEnum(LinkState, name="linkstate", create_constraint=False),
    ...
    theme_preset: Mapped[str] = mapped_column(String(50), server_default="default", nullable=False)
    noindex: Mapped[bool] = mapped_column(Boolean(), server_default="true", nullable=False)
    allow_comments: Mapped[bool] = mapped_column(Boolean(), server_default="false", nullable=False)
    ...
    share: Mapped["Share"] = relationship("Share", backref="published_links")
    events: Mapped[list["PublishedLinkEvent"]] = relationship("PublishedLinkEvent", back_populates="link", cascade="all, delete-orphan")
...
class PublishedLinkEvent(Base):
    ...
    published_link_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("published_links.id", ondelete="CASCADE"), nullable=False, index=True,
    )
    event_type: Mapped[str] = mapped_column(String(50), nullable=False, index=True)
    actor_user_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True,
    )
    actor_kind: Mapped[str] = mapped_column(String(20), server_default="user", nullable=False)
    ...
    link: Mapped["PublishedLink"] = relationship("PublishedLink", back_populates="events")
...
class CommentThread(Base, TimestampMixin):
    ...
    published_link_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("published_links.id", ondelete="SET NULL"), nullable=True,
    )
    target_id: Mapped[str] = mapped_column(String(512), nullable=False)
    anchor_type: Mapped[CommentAnchorType] = mapped_column(
        PgEnum(CommentAnchorType, name="commentanchortype", create_constraint=False),
    ...
    status: Mapped[CommentThreadStatus] = mapped_column(
        PgEnum(CommentThreadStatus, name="commentthreadstatus", create_constraint=False),
    ...
    items: Mapped[list["CommentItem"]] = relationship("CommentItem", back_populates="thread", cascade="all, delete-orphan")

    __table_args__ = (Index("ix_ct_share_target", "share_id", "target_id"),)
...
class CommentItem(Base):
    ...
    thread_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("comment_threads.id", ondelete="CASCADE"), nullable=False, index=True,
    )
    ...
    thread: Mapped["CommentThread"] = relationship("CommentThread", back_populates="items")
