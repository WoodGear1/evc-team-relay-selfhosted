...
class AuditAction(str, Enum):
    ...
    LINK_ACCESS_GRANTED = "link_access_granted"
    LINK_ACCESS_DENIED = "link_access_denied"
    # Comments
    COMMENT_THREAD_CREATED = "comment_thread_created"
    ...
    WEB_SESSION_CREATED = "web_session_created"
...
class DocumentVersion(Base, TimestampMixin):
    ...
    __table_args__ = (
        Index("ix_document_versions_share_path_created", "share_id", "document_path", "created_at"),
    )
    ...
    document_path: Mapped[str] = mapped_column(String(1024), nullable=False, index=True)
...
# ---------------------------------------------------------------------------
# Published Links — separate web-access entities decoupled from Share
# ---------------------------------------------------------------------------


class PublishedLink(Base, TimestampMixin):
    __tablename__ = "published_links"
    __table_args__ = (
        Index("ix_published_links_slug", "slug", unique=True),
        Index("ix_published_links_share_target", "share_id", "target_type", "target_id"),
    )
    ...
    target_type: Mapped[str] = mapped_column(String(10), nullable=False)  # "file" | "folder"
    target_id: Mapped[str] = mapped_column(
        String(512), nullable=False,
    )
    target_path: Mapped[str] = mapped_column(String(1024), nullable=False)

    access_mode: Mapped[LinkAccessMode] = mapped_column(
        PgEnum(
            LinkAccessMode,
            name="linkaccessmode",
            values_callable=lambda x: [e.value for e in x],
    ...
    noindex: Mapped[bool] = mapped_column(default=True, nullable=False)
    allow_comments: Mapped[bool] = mapped_column(default=False, nullable=False)
    ...
    # Page-level custom rendering metadata (JSONB)
    ...
    events: Mapped[list["PublishedLinkEvent"]] = relationship(
        back_populates="published_link",
        cascade="all, delete-orphan",
    )
...
class PublishedLinkEvent(Base):
    __tablename__ = "published_link_events"
    __table_args__ = (
        Index("ix_ple_link_created", "published_link_id", "created_at"),
    )

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    published_link_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("published_links.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    target_type: Mapped[str | None] = mapped_column(String(10), nullable=True)
    ...
    published_link: Mapped["PublishedLink"] = relationship(back_populates="events")
    actor: Mapped["User | None"] = relationship()


# ---------------------------------------------------------------------------
# Comments — threads and items, anchored to target (not individual links)
# ---------------------------------------------------------------------------


class CommentThread(Base, TimestampMixin):
    """A thread of comments anchored to a target document/block within a share.

    Comments belong to the target (share + target_id), not to a specific
    published link. The ``published_link_id`` is optional and only stored as
    the web entry-point through which the thread was created; it does NOT
    restrict visibility across links pointing to the same target.
    """

    __tablename__ = "comment_threads"
    __table_args__ = (
        Index("ix_ct_share_target", "share_id", "target_id"),
    )
    ...
    published_link_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("published_links.id", ondelete="SET NULL"),
        nullable=True,
    )
    ...
    anchor_type: Mapped[CommentAnchorType] = mapped_column(
        PgEnum(
            CommentAnchorType,
            name="commentanchortype",
            values_callable=lambda x: [e.value for e in x],
        ),
        nullable=False,
        default=CommentAnchorType.DOCUMENT,
    )
    anchor_id: Mapped[str | None] = mapped_column(
        String(255), nullable=True,
    )  # block-id when anchor_type == BLOCK
    ...
    share: Mapped["Share"] = relationship()
    published_link: Mapped["PublishedLink | None"] = relationship(back_populates="comment_threads")
    creator: Mapped["User | None"] = relationship(foreign_keys=[created_by])
    resolver: Mapped["User | None"] = relationship(foreign_keys=[resolved_by])
    items: Mapped[list["CommentItem"]] = relationship(
        back_populates="thread",
        cascade="all, delete-orphan",
        order_by="CommentItem.created_at",
    )
...
class CommentItem(Base):
    ...
    thread: Mapped["CommentThread"] = relationship(back_populates="items")
