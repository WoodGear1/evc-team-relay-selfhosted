...
export class Document extends HasProvider implements IFile, HasMimeType {
	...
	private pendingLocalEditMarkers = 0;
	...
	constructor(
		...
		const remoteChangeObserver = (event: Y.YTextEvent) => {
			if (event.changes.delta.length === 0) {
				return;
			}
			if (this.consumeLocalEditMarker()) {
				this.clearRemoteChange();
				return;
			}
			const origin = event.transaction.origin;
			...
			this.markRemoteChange();
		};
		this.ytext.observe(remoteChangeObserver);
		this.unsubscribes.push(() => {
			this.ytext.unobserve(remoteChangeObserver);
		});
	...
	markLocalEdit(): void {
		this.pendingLocalEditMarkers += 1;
		this.clearRemoteChange();
	}

	private consumeLocalEditMarker(): boolean {
		if (this.pendingLocalEditMarkers < 1) {
			return false;
		}
		this.pendingLocalEditMarkers -= 1;
		return true;
