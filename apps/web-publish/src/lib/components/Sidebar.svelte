...
<aside class="sidebar" class:open={menuOpen} class:collapsed={isCollapsed}>
	...
	<div class="sidebar-footer">
		<div class="theme-switcher" class:collapsed={isCollapsed}>
			{#if !isCollapsed}
				<p class="theme-switcher-label">Theme</p>
			{/if}
			<div
			...
			>
				{#each themeOptions as option}
					...
					>
						{isCollapsed ? option.shortLabel : option.label}
					</button>
				{/each}
			</div>
			{#if !isCollapsed}
				<p class="theme-switcher-hint">Active: {resolvedTheme}</p>
			{/if}
		</div>
		{#if !isCollapsed}
			<p class="powered-by">Published with your relay</p>
		{/if}
	</div>
</aside>
