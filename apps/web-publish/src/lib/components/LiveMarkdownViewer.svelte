<script lang="ts">
	...
	import { getRelayToken, type RelayToken, type WebResourceKind } from '$lib/api';
	...
	interface Props {
		...
		sessionToken?: string;
		/** Whether the public page is backed by a legacy share or a published link */
		resourceKind?: WebResourceKind;
		class?: string;
		folderItems?: Array<{ path: string; name: string; type: string; content?: string }>;
	}

	let { content, docId, slug, sessionToken, resourceKind = 'share', class: className = '', folderItems }: Props = $props();
	...
	onMount(async () => {
		...
		try {
			...
			const token = await getRelayToken(slug, sessionToken, resourceKind);
...
<style>
	...
	.live-indicator {
		...
		gap: 0.5rem;
		padding: 0.5rem 1rem;
		background-color: hsl(var(--success) / 0.92);
