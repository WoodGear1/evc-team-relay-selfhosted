...
export interface LinearPageLink {
	name: string;
	path: string;
}
...
export function getLinearDocumentOrder(items: FolderItem[]): LinearPageLink[] {
	const tree = buildFileTree(items);
	const ordered: LinearPageLink[] = [];

	const visit = (node: TreeNode) => {
		if (node.type === 'doc' || node.type === 'canvas') {
			ordered.push({
				name: node.name,
				path: node.path
			});
...
export function getPrevNextForPath(
	items: FolderItem[],
	currentPath: string
): { previous: LinearPageLink | null; next: LinearPageLink | null } {
	const ordered = getLinearDocumentOrder(items);
	const index = ordered.findIndex((entry) => entry.path === currentPath);
